...
void WinMTRNet::DoTrace(int address)
{
	...
	wmtrdlg->Transit(wmtrdlg->STOPPING);
}
...
void TraceThread(void *p)
{
	...
	int             countsize                                   = wmtrnet->wmtrdlg->countsize;
	int tracecount = 0;
    /*
    ...
    while(wmtrnet->tracing && tracecount < countsize) {
...
int WinMTRNet::GetIP(int at, char *a)
{
	WaitForSingleObject(ghMutex, INFINITE);
	 {
		int addr = GetAddr(at);
		if (addr == 0){
			strcpy(a, "No response");
		}
		else{
			sprintf(a, "%d.%d.%d.%d",
				(addr >> 24) & 0xff,
...
int WinMTRNet::GetName(int at, char *n)
{
	...
	if (!strcmp(host[at].name, "")) {
		int addr = GetAddr(at);
		if (addr == 0)
			strcpy(n, "No response from host");
	}
	else {
